...
Add users as owners to your Service. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_service_user_owner" "by_name" {
  name = "example-service_user_owner"
}
