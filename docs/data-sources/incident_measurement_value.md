...
The computed value of one incident measurement for one incident, recomputed from the incident's timeline rather than accumulated Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_measurement_value" "by_name" {
  name = "example-incident_measurement_value"
}
...
- `incident_measurement_id` (String) A unique identifier for an object, represented as a UUID.. Computed.
...
- `start_incident_state_timeline_id` (String) A unique identifier for an object, represented as a UUID.. Computed.
- `end_incident_state_timeline_id` (String) A unique identifier for an object, represented as a UUID.. Computed.
