...
Add teams as owners to your serverless functions. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_serverless_function_team_owner" "by_name" {
  name = "example-serverless_function_team_owner"
}
...
- `serverless_function_id` (String) A unique identifier for an object, represented as a UUID.. Computed.
