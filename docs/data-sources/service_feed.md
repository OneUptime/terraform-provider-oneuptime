...
Log of everything that happened to this service - creation, updates, owner changes and the rules that made them. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_service_feed" "by_name" {
  name = "example-service_feed"
}
...
- `feed_info_in_markdown` (String) Log of the service change in Markdown.. Computed.
- `more_information_in_markdown` (String) More information in Markdown.. Computed.
- `service_feed_event_type` (String) Service Feed Event.. Computed.
