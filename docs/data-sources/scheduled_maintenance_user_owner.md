...
Add users as owners to your scheduled maintenance event. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_scheduled_maintenance_user_owner" "by_name" {
  name = "example-scheduled_maintenance_user_owner"
}
