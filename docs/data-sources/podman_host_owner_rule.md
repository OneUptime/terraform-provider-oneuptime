...
Configure rules for automatically assigning owner users and teams when matching Podman hosts are created Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_podman_host_owner_rule" "by_name" {
  name = "example-podman_host_owner_rule"
}
...
- `description` (String) Description of this Podman host owner rule.. Computed.
...
- `podman_host_labels` (Set) Only trigger for Podman hosts that have at least one of these labels. Leave empty to match regardless of labels... Computed.
...
- `owner_users` (Set) Users to add as owners on the Podman host when this rule matches... Computed.
- `owner_teams` (Set) Teams to add as owners on the Podman host when this rule matches... Computed.
