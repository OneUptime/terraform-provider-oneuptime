...
Add teams as owners to your Kubernetes clusters. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_kubernetes_cluster_team_owner" "by_name" {
  name = "example-kubernetes_cluster_team_owner"
}
