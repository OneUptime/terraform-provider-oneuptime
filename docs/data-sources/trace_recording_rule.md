---
page_title: "oneuptime_trace_recording_rule Data Source - oneuptime"
subcategory: "Other"
description: |-
...
# oneuptime_trace_recording_rule (Data Source)

Derived metrics computed on a schedule from aggregations over spans. Results are written into the metric store as a new series. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_trace_recording_rule" "by_name" {
  name = "example-trace_recording_rule"
}

data "oneuptime_trace_recording_rule" "by_id" {
