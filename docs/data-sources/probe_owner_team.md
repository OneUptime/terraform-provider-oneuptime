...
Add teams as owners to your probes. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_probe_owner_team" "by_name" {
  name = "example-probe_owner_team"
}
