...
Add users as owners to your Ceph clusters. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_ceph_cluster_user_owner" "by_name" {
  name = "example-ceph_cluster_user_owner"
}
