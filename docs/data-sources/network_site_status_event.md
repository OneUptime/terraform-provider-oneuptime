...
History of the rolled-up health status of a Network Site (Operational to Offline for example) Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_network_site_status_event" "by_name" {
  name = "example-network_site_status_event"
}
