...
Add teams as owners to your incident episodes. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_episode_team_owner" "by_name" {
  name = "example-incident_episode_team_owner"
}
