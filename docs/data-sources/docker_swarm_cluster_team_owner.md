...
Add teams as owners to your DockerSwarm clusters. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_docker_swarm_cluster_team_owner" "by_name" {
  name = "example-docker_swarm_cluster_team_owner"
}
