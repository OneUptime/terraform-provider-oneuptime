...
Add users as owners to your serverless functions. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_serverless_function_user_owner" "by_name" {
  name = "example-serverless_function_user_owner"
}
