...
Add teams as owners to your Scheduled Maintenance event. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_scheduled_maintenance_team_owner" "by_name" {
  name = "example-scheduled_maintenance_team_owner"
}
