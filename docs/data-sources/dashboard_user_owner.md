...
Add users as owners to your dashboards. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_dashboard_user_owner" "by_name" {
  name = "example-dashboard_user_owner"
}
