...
Configure rules for automatically assigning owner users and teams when matching status pages are created Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_status_page_owner_rule" "by_name" {
  name = "example-status_page_owner_rule"
}
...
- `status_page_labels` (Set) Only trigger for status pages that have at least one of these labels. Leave empty to match regardless of labels... Computed.
...
- `owner_users` (Set) Users to add as owners on the status page when this rule matches... Computed.
- `owner_teams` (Set) Teams to add as owners on the status page when this rule matches... Computed.
