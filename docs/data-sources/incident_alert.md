...
Alerts linked to an incident. Link alerts to an incident to track them as part of that incident's response. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_alert" "by_name" {
  name = "example-incident_alert"
}
