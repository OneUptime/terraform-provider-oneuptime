...
Add users as owners to your AI agents. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_ai_agent_user_owner" "by_name" {
  name = "example-ai_agent_user_owner"
}
