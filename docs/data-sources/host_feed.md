...
Log of everything that happened to this host - creation, updates, owner changes and the rules that made them. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_host_feed" "by_name" {
  name = "example-host_feed"
}
...
- `feed_info_in_markdown` (String) Log of the host change in Markdown.. Computed.
- `more_information_in_markdown` (String) More information in Markdown.. Computed.
- `host_feed_event_type` (String) Host Feed Event.. Computed.
