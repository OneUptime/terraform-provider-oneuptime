...
Log of the entire scheduled maintenance state change. This is a log of all the scheduled maintenance state changes, public notes, more etc. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_scheduled_maintenance_feed" "by_name" {
  name = "example-scheduled_maintenance_feed"
}
...
- `feed_info_in_markdown` (String) Log of the entire scheduled maintenance state change in Markdown.. Computed.
- `more_information_in_markdown` (String) More information in Markdown.. Computed.
- `scheduled_maintenance_feed_event_type` (String) ScheduledMaintenance Log Event.. Computed.
