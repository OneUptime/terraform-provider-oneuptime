...
Configure the levels of your network site hierarchy (Region, Market, Unit and so on). Rename them, reorder them, or add your own. Look up by `id` or by `name` (must match exactly one item).
...
- `order` (Number) Where this type sits in the site hierarchy. Lower numbers are higher up: Region is 2 and Unit is 5, for example... Computed.
