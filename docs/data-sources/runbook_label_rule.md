...
Configure rules for automatically attaching labels to runbooks when matching runbooks are created Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_runbook_label_rule" "by_name" {
  name = "example-runbook_label_rule"
}
...
- `description` (String) Description of this runbook label rule.. Computed.
- `is_enabled` (Bool) Whether this rule is enabled.. Computed.
- `runbook_labels` (Set) Only trigger for runbooks that already have at least one of these labels. Leave empty to match regardless of labels... Computed.
...
- `labels_to_add` (Set) Labels to attach to the runbook when this rule matches. Already-attached labels are not duplicated... Computed.
