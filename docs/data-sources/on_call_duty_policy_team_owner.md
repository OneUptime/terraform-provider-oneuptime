...
Add teams as owners to your onCallDutyPolicys. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_on_call_duty_policy_team_owner" "by_name" {
  name = "example-on_call_duty_policy_team_owner"
}
