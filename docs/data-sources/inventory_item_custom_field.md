...
Manage custom fields on your inventory items Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_inventory_item_custom_field" "by_name" {
  name = "example-inventory_item_custom_field"
}
