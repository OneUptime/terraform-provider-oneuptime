...
Log of the entire incident episode activity. This is a log of all the episode state changes, incidents added/removed, notes, etc. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_episode_feed" "by_name" {
  name = "example-incident_episode_feed"
}
...
- `feed_info_in_markdown` (String) Log of the entire incident episode activity in Markdown.. Computed.
- `more_information_in_markdown` (String) More information in Markdown.. Computed.
- `incident_episode_feed_event_type` (String) Incident Episode Feed Event Type.. Computed.
