...
Configure rules to drop or sample spans before storage to reduce volume and cost. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_trace_drop_filter" "by_name" {
  name = "example-trace_drop_filter"
}
...
- `filter_query` (String) Filter expression that identifies which spans to drop or sample... Computed.
