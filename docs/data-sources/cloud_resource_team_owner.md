...
Add teams as owners to your cloud resources. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_cloud_resource_team_owner" "by_name" {
  name = "example-cloud_resource_team_owner"
}
