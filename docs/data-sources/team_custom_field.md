...
Manage custom fields for your teams Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_team_custom_field" "by_name" {
  name = "example-team_custom_field"
}
...
- `custom_field_type` (String) Is this field Text, Number, Boolean or Dropdown?.. Computed.
