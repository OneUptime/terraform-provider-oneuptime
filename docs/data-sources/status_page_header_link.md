...
Manage header links on your status page Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_status_page_header_link" "by_name" {
  name = "example-status_page_header_link"
}
