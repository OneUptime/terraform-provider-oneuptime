...
Add teams as owners to your dashboards. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_dashboard_team_owner" "by_name" {
  name = "example-dashboard_team_owner"
}
