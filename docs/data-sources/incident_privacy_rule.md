...
Configure rules for automatically marking matching incidents as private Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_privacy_rule" "by_name" {
  name = "example-incident_privacy_rule"
}
...
- `description` (String) Description of this incident privacy rule.. Computed.
