...
Add teams as owners to your Service Level Objectives. Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_service_level_objective_team_owner" "by_name" {
  name = "example-service_level_objective_team_owner"
}
