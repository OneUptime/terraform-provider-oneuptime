...
Configure rules for automatically attaching labels to incident episodes when matching episodes are created Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_episode_label_rule" "by_name" {
  name = "example-incident_episode_label_rule"
}
...
- `description` (String) Description of this incident episode label rule.. Computed.
