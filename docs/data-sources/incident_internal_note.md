...
Manage internal notes for your incident Look up by `id` or by `name` (must match exactly one item).
...
```terraform
data "oneuptime_incident_internal_note" "by_name" {
  name = "example-incident_internal_note"
}
