...
- `proxmox_cluster_labels` (Set) Only trigger for Proxmox clusters that already have at least one of these labels. Leave empty to match regardless of labels...
...
- `labels_to_add` (Set) Labels to attach to the Proxmox cluster when this rule matches. Already-attached labels are not duplicated...
