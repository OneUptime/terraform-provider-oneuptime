...
```terraform
resource "oneuptime_podman_host_owner_rule" "example" {
...
- `podman_host_labels` (Set) Only trigger for Podman hosts that have at least one of these labels. Leave empty to match regardless of labels...
...
- `owner_users` (Set) Users to add as owners on the Podman host when this rule matches...
- `owner_teams` (Set) Teams to add as owners on the Podman host when this rule matches...
...
```shell
terraform import oneuptime_podman_host_owner_rule.example <id>
```
