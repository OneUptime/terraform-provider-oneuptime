...
- `is_archived` (Bool) Archived SLOs are hidden from lists and are not evaluated...
...
- `auto_added_monitors` (Set) Monitors that were attached to this SLO by its monitor rules rather than by hand. Maintained by the server...
