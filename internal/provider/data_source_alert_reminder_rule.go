...
var _ datasource.DataSource = &AlertReminderRuleDataSource{}

func NewAlertReminderRuleDataSource() datasource.DataSource {
    return &AlertReminderRuleDataSource{}
}

// AlertReminderRuleDataSource defines the data source implementation.
type AlertReminderRuleDataSource struct {
    client *Client
}
...
func (d *AlertReminderRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *AlertReminderRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AlertReminderRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data AlertReminderRuleDataSourceModel
    ...
    if hasId {
        readPath := "/alert-reminder-rule/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read alert_reminder_rule: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/alert-reminder-rule/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list alert_reminder_rule, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list alert_reminder_rule: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for alert_reminder_rule.")
            return
        }
        item = first
    }
