...
func (d *ScheduledMaintenanceLabelRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Configure rules for automatically attaching labels to scheduled maintenance events — including labels inherited from the event's monitors — when matching events are created Look up an existing scheduled_maintenance_label_rule by `id` or by `name`.",
...
func (d *ScheduledMaintenanceLabelRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a scheduled_maintenance_label_rule.",
        )
        return
    }
    ...
    if hasId {
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_maintenance_label_rule found with id %q.", data.Id.ValueString()))
            return
        }
        ...
        httpResp, err := d.client.Post(ctx, "/scheduled-maintenance-label-rule/get-list", listBody)
        if err != nil {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_maintenance_label_rule found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one scheduled_maintenance_label_rule matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
