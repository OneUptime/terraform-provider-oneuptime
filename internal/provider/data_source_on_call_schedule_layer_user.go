...
var _ datasource.DataSource = &OnCallScheduleLayerUserDataSource{}

func NewOnCallScheduleLayerUserDataSource() datasource.DataSource {
    return &OnCallScheduleLayerUserDataSource{}
}

// OnCallScheduleLayerUserDataSource defines the data source implementation.
type OnCallScheduleLayerUserDataSource struct {
    client *Client
}
...
func (d *OnCallScheduleLayerUserDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *OnCallScheduleLayerUserDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *OnCallScheduleLayerUserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data OnCallScheduleLayerUserDataSourceModel
    ...
    if hasId {
        readPath := "/on-call-duty-schedule-layer-user/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read on_call_schedule_layer_user: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list on_call_schedule_layer_user: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for on_call_schedule_layer_user.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["onCallDutyPolicyScheduleLayerId"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["onCallDutyPolicyScheduleLayerId"].(string); ok {
        data.OnCallDutyPolicyScheduleLayerId = types.StringValue(val)
    } else {
        data.OnCallDutyPolicyScheduleLayerId = types.StringNull()
    }
