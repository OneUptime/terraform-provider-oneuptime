...
func (d *IncidentPublicNoteDataDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if val, ok := incidentPublicNoteDataResponse["attachments"].([]interface{}); ok {
    ...
    if val, ok := incidentPublicNoteDataResponse["subscriber_notification_status_message"].(string); ok {
    ...
    if val, ok := incidentPublicNoteDataResponse["posted_from_slack_message_id"].(string); ok {
