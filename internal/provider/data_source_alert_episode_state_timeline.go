...
var _ datasource.DataSource = &AlertEpisodeStateTimelineDataSource{}

func NewAlertEpisodeStateTimelineDataSource() datasource.DataSource {
    return &AlertEpisodeStateTimelineDataSource{}
}

// AlertEpisodeStateTimelineDataSource defines the data source implementation.
type AlertEpisodeStateTimelineDataSource struct {
    client *Client
}

// AlertEpisodeStateTimelineDataSourceModel describes the data source data model.
type AlertEpisodeStateTimelineDataSourceModel struct {
...
func (d *AlertEpisodeStateTimelineDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_alert_episode_state_timeline"
}

func (d *AlertEpisodeStateTimelineDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Change state of the alert episodes (Created to Acknowledged for example) Look up an existing alert_episode_state_timeline by `id` or by `name`.",
...
func (d *AlertEpisodeStateTimelineDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AlertEpisodeStateTimelineDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data AlertEpisodeStateTimelineDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a alert_episode_state_timeline.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/alert-episode-state-timeline/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No alert_episode_state_timeline found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read alert_episode_state_timeline: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list alert_episode_state_timeline: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No alert_episode_state_timeline found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one alert_episode_state_timeline matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for alert_episode_state_timeline.")
            return
        }
        item = first
    }
