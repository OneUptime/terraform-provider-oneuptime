...
var _ datasource.DataSource = &MonitorFeedDataSource{}

func NewMonitorFeedDataSource() datasource.DataSource {
    return &MonitorFeedDataSource{}
}

// MonitorFeedDataSource defines the data source implementation.
type MonitorFeedDataSource struct {
    client *Client
}

// MonitorFeedDataSourceModel describes the data source data model.
type MonitorFeedDataSourceModel struct {
...
func (d *MonitorFeedDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_monitor_feed"
}

func (d *MonitorFeedDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Log of the entire monitor state change. This is a log of all the monitor state changes, public notes, more etc. Look up an existing monitor_feed by `id` or by `name`.",
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "feed_info_in_markdown": schema.StringAttribute{
                MarkdownDescription: "Log of the entire monitor state change in Markdown.",
                Computed: true,
            },
...
func (d *MonitorFeedDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MonitorFeedDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data MonitorFeedDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a monitor_feed.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/monitor-feed/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No monitor_feed found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read monitor_feed: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/monitor-feed/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list monitor_feed, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list monitor_feed: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No monitor_feed found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one monitor_feed matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for monitor_feed.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["monitorFeedEventType"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
            data.MonitorFeedEventType = types.StringValue(val)
        } else if val, ok := obj["value"].(string); ok {
            data.MonitorFeedEventType = types.StringValue(val)
        } else if val, ok := obj["value"].(float64); ok {
            data.MonitorFeedEventType = types.StringValue(fmt.Sprintf("%v", val))
        } else if jsonBytes, err := json.Marshal(obj); err == nil {
            data.MonitorFeedEventType = types.StringValue(string(jsonBytes))
        } else {
            data.MonitorFeedEventType = types.StringNull()
        }
    } else if val, ok := item["monitorFeedEventType"].(string); ok {
        data.MonitorFeedEventType = types.StringValue(val)
    } else {
        data.MonitorFeedEventType = types.StringNull()
    }
