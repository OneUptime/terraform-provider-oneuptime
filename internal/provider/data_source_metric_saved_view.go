...
func (d *MetricSavedViewDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Save and reuse metrics explorer views, including the current search, filters, time range, and page size. Look up an existing metric_saved_view by `id` or by `name`.",
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "query": schema.StringAttribute{
                MarkdownDescription: "Serialized metrics explorer view state (search, filters, time range, page size) for this saved view..",
                Computed: true,
            },
...
func (d *MetricSavedViewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a metric_saved_view.",
        )
        return
    }
    ...
    if hasId {
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No metric_saved_view found with id %q.", data.Id.ValueString()))
            return
        }
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No metric_saved_view found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one metric_saved_view matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
