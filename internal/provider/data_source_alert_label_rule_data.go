...
var _ datasource.DataSource = &AlertLabelRuleDataDataSource{}

func NewAlertLabelRuleDataDataSource() datasource.DataSource {
    return &AlertLabelRuleDataDataSource{}
}

// AlertLabelRuleDataDataSource defines the data source implementation.
type AlertLabelRuleDataDataSource struct {
    client *Client
}

// AlertLabelRuleDataDataSourceModel describes the data source data model.
type AlertLabelRuleDataDataSourceModel struct {
...
func (d *AlertLabelRuleDataDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_alert_label_rule_data"
}

func (d *AlertLabelRuleDataDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "alert_label_rule_data data source",
...
func (d *AlertLabelRuleDataDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AlertLabelRuleDataDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data AlertLabelRuleDataDataSourceModel
    ...
    // Build API path
    apiPath := "/" + "alert-label-rule" + "/" + data.Id.ValueString() + "/" + "get-item"
    ...
    if err != nil {
        resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read alert_label_rule_data, got error: %s", err))
        return
    }

    var alertLabelRuleDataResponse map[string]interface{}
    err = d.client.ParseResponse(httpResp, &alertLabelRuleDataResponse)
    if err != nil {
        resp.Diagnostics.AddError("Parse Error", fmt.Sprintf("Unable to parse alert_label_rule_data response, got error: %s", err))
        return
    }

    // Extract data from response
    if dataMap, ok := alertLabelRuleDataResponse["data"].(map[string]interface{}); ok {
        alertLabelRuleDataResponse = dataMap
    }

    // Update the model with response data
    if val, ok := alertLabelRuleDataResponse["id"].(string); ok {
        data.Id = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["name"].(string); ok {
        data.Name = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["created_at"].(string); ok {
        data.CreatedAt = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["updated_at"].(string); ok {
        data.UpdatedAt = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["deleted_at"].(string); ok {
        data.DeletedAt = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["version"].(float64); ok {
        data.Version = types.NumberValue(big.NewFloat(val))
    }
    if val, ok := alertLabelRuleDataResponse["project_id"].(string); ok {
        data.ProjectId = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["description"].(string); ok {
        data.Description = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["is_enabled"].(bool); ok {
        data.IsEnabled = types.BoolValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["monitors"].([]interface{}); ok {
    ...
    if val, ok := alertLabelRuleDataResponse["alert_severities"].([]interface{}); ok {
        elements := make([]attr.Value, len(val))
        for i, item := range val {
            if strItem, ok := item.(string); ok {
                elements[i] = types.StringValue(strItem)
    ...
    if val, ok := alertLabelRuleDataResponse["alert_labels"].([]interface{}); ok {
        elements := make([]attr.Value, len(val))
        for i, item := range val {
            if strItem, ok := item.(string); ok {
                elements[i] = types.StringValue(strItem)
    ...
    if val, ok := alertLabelRuleDataResponse["monitor_labels"].([]interface{}); ok {
        elements := make([]attr.Value, len(val))
        for i, item := range val {
            if strItem, ok := item.(string); ok {
                elements[i] = types.StringValue(strItem)
    ...
    if val, ok := alertLabelRuleDataResponse["alert_title_pattern"].(string); ok {
        data.AlertTitlePattern = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["alert_description_pattern"].(string); ok {
        data.AlertDescriptionPattern = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["monitor_name_pattern"].(string); ok {
        data.MonitorNamePattern = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["monitor_description_pattern"].(string); ok {
        data.MonitorDescriptionPattern = types.StringValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["labels_to_add"].([]interface{}); ok {
        elements := make([]attr.Value, len(val))
        for i, item := range val {
            if strItem, ok := item.(string); ok {
                elements[i] = types.StringValue(strItem)
    ...
    if val, ok := alertLabelRuleDataResponse["inherit_labels_from_monitors"].(bool); ok {
        data.InheritLabelsFromMonitors = types.BoolValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["inherit_labels_from_hosts"].(bool); ok {
        data.InheritLabelsFromHosts = types.BoolValue(val)
    }
    if val, ok := alertLabelRuleDataResponse["inherit_labels_from_kubernetes_clusters"].(bool); ok {
    ...
    if val, ok := alertLabelRuleDataResponse["created_by_user_id"].(string); ok {
