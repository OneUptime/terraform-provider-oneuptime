...
var _ datasource.DataSource = &ScheduledMaintenanceOwnerRuleDataSource{}

func NewScheduledMaintenanceOwnerRuleDataSource() datasource.DataSource {
    return &ScheduledMaintenanceOwnerRuleDataSource{}
}

// ScheduledMaintenanceOwnerRuleDataSource defines the data source implementation.
type ScheduledMaintenanceOwnerRuleDataSource struct {
    client *Client
}

// ScheduledMaintenanceOwnerRuleDataSourceModel describes the data source data model.
type ScheduledMaintenanceOwnerRuleDataSourceModel struct {
...
func (d *ScheduledMaintenanceOwnerRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_scheduled_maintenance_owner_rule"
}

func (d *ScheduledMaintenanceOwnerRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Configure rules for automatically assigning owner users and teams when matching scheduled maintenance events are created Look up an existing scheduled_maintenance_owner_rule by `id` or by `name`.",
...
func (d *ScheduledMaintenanceOwnerRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ScheduledMaintenanceOwnerRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data ScheduledMaintenanceOwnerRuleDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a scheduled_maintenance_owner_rule.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/scheduled-maintenance-owner-rule/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_maintenance_owner_rule found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read scheduled_maintenance_owner_rule: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list scheduled_maintenance_owner_rule: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_maintenance_owner_rule found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one scheduled_maintenance_owner_rule matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for scheduled_maintenance_owner_rule.")
            return
        }
        item = first
    }
