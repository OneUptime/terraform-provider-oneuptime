...
func (d *WebAuthnCredentialDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "WebAuthn credentials for users (security keys) Look up an existing web_authn_credential by `id` or by `name`.",
...
func (d *WebAuthnCredentialDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a web_authn_credential.",
        )
        return
    }
    ...
    if hasId {
        resp.Diagnostics.AddError("Lookup Not Supported", "web_authn_credential cannot be looked up by id: the API exposes no get endpoint. Use the name filter instead.")
        return
    } else {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No web_authn_credential found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one web_authn_credential matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
