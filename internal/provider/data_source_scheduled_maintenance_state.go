...
var _ datasource.DataSource = &ScheduledMaintenanceStateDataSource{}

func NewScheduledMaintenanceStateDataSource() datasource.DataSource {
    return &ScheduledMaintenanceStateDataSource{}
}

// ScheduledMaintenanceStateDataSource defines the data source implementation.
type ScheduledMaintenanceStateDataSource struct {
    client *Client
}
...
func (d *ScheduledMaintenanceStateDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *ScheduledMaintenanceStateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ScheduledMaintenanceStateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data ScheduledMaintenanceStateDataSourceModel
    ...
    if hasId {
        readPath := "/scheduled-maintenance-state/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read scheduled_maintenance_state: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/scheduled-maintenance-state/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list scheduled_maintenance_state, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list scheduled_maintenance_state: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for scheduled_maintenance_state.")
            return
        }
        item = first
    }
