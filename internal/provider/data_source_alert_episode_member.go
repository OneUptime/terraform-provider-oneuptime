...
func (d *AlertEpisodeMemberDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Link between alerts and episodes Look up an existing alert_episode_member by `id` or by `name`.",
...
func (d *AlertEpisodeMemberDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a alert_episode_member.",
        )
        return
    }
    ...
    if hasId {
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No alert_episode_member found with id %q.", data.Id.ValueString()))
            return
        }
        ...
        httpResp, err := d.client.Post(ctx, "/alert-episode-member/get-list", listBody)
        if err != nil {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No alert_episode_member found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one alert_episode_member matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
