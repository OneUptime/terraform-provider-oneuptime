...
var _ datasource.DataSource = &MonitorGroupResourceDataSource{}

func NewMonitorGroupResourceDataSource() datasource.DataSource {
    return &MonitorGroupResourceDataSource{}
}

// MonitorGroupResourceDataSource defines the data source implementation.
type MonitorGroupResourceDataSource struct {
    client *Client
}
...
func (d *MonitorGroupResourceDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *MonitorGroupResourceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MonitorGroupResourceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data MonitorGroupResourceDataSourceModel
    ...
    if hasId {
        readPath := "/monitor-group-resource/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read monitor_group_resource: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/monitor-group-resource/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list monitor_group_resource, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list monitor_group_resource: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for monitor_group_resource.")
            return
        }
        item = first
    }
