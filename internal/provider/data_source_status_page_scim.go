...
var _ datasource.DataSource = &StatusPageScimDataSource{}

func NewStatusPageScimDataSource() datasource.DataSource {
    return &StatusPageScimDataSource{}
}

// StatusPageScimDataSource defines the data source implementation.
type StatusPageScimDataSource struct {
    client *Client
}
...
func (d *StatusPageScimDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *StatusPageScimDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data StatusPageScimDataSourceModel
    ...
    if hasId {
        readPath := "/status-page-scim/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read status_page_scim: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list status_page_scim: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for status_page_scim.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["bearerToken"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["bearerToken"].(string); ok {
        data.BearerToken = types.StringValue(val)
    } else {
        data.BearerToken = types.StringNull()
    }
    if val, ok := item["autoProvisionUsers"].(bool); ok {
        data.AutoProvisionUsers = types.BoolValue(val)
    } else {
        data.AutoProvisionUsers = types.BoolNull()
    }
    if val, ok := item["autoDeprovisionUsers"].(bool); ok {
        data.AutoDeprovisionUsers = types.BoolValue(val)
    } else {
        data.AutoDeprovisionUsers = types.BoolNull()
    }
