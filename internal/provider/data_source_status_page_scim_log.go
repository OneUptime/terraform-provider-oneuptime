...
func (d *StatusPageScimLogDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Logs of all SCIM provisioning operations for status pages. Look up an existing status_page_scim_log by `id` or by `name`.",
...
func (d *StatusPageScimLogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a status_page_scim_log.",
        )
        return
    }
    ...
    if hasId {
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No status_page_scim_log found with id %q.", data.Id.ValueString()))
            return
        }
        ...
        httpResp, err := d.client.Post(ctx, "/status-page-scim-log/get-list", listBody)
        if err != nil {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No status_page_scim_log found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one status_page_scim_log matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
