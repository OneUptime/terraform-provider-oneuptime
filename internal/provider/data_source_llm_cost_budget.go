...
func (d *LlmCostBudgetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a llm_cost_budget.",
        )
        return
    }
    ...
    if hasId {
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No llm_cost_budget found with id %q.", data.Id.ValueString()))
            return
        }
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No llm_cost_budget found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one llm_cost_budget matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
