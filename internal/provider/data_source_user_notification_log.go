...
func (d *UserNotificationLogDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Log events for user notifications Look up an existing user_notification_log by `id` or by `name`.",
...
func (d *UserNotificationLogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a user_notification_log.",
        )
        return
    }
    ...
    if hasId {
        resp.Diagnostics.AddError("Lookup Not Supported", "user_notification_log cannot be looked up by id: the API exposes no get endpoint. Use the name filter instead.")
        return
    } else {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No user_notification_log found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one user_notification_log matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
