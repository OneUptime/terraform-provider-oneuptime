...
var _ datasource.DataSource = &PushNotificationLogDataSource{}

func NewPushNotificationLogDataSource() datasource.DataSource {
    return &PushNotificationLogDataSource{}
}

// PushNotificationLogDataSource defines the data source implementation.
type PushNotificationLogDataSource struct {
    client *Client
}

// PushNotificationLogDataSourceModel describes the data source data model.
type PushNotificationLogDataSourceModel struct {
...
func (d *PushNotificationLogDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_push_notification_log"
}

func (d *PushNotificationLogDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Logs of all the Push Notifications sent out to all users and subscribers for this project. Look up an existing push_notification_log by `id` or by `name`.",
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "title": schema.StringAttribute{
                MarkdownDescription: "Title of the push notification.",
                Computed: true,
            },
            "body": schema.StringAttribute{
                MarkdownDescription: "Body of the push notification.",
                Computed: true,
            },
            "device_type": schema.StringAttribute{
                MarkdownDescription: "Type of device this was sent to (e.g., web).",
                Computed: true,
            },
            "device_name": schema.StringAttribute{
                MarkdownDescription: "Name of the device this was sent to.",
                Computed: true,
            },
            ...
            "status": schema.StringAttribute{
                MarkdownDescription: "Status of the push notification.",
                Computed: true,
            },
...
func (d *PushNotificationLogDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *PushNotificationLogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data PushNotificationLogDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a push_notification_log.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/push-notification-log/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No push_notification_log found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read push_notification_log: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/push-notification-log/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list push_notification_log, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list push_notification_log: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No push_notification_log found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one push_notification_log matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for push_notification_log.")
            return
        }
        item = first
    }
