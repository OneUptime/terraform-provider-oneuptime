...
var _ datasource.DataSource = &IncidentEpisodeInternalNoteDataSource{}

func NewIncidentEpisodeInternalNoteDataSource() datasource.DataSource {
    return &IncidentEpisodeInternalNoteDataSource{}
}

// IncidentEpisodeInternalNoteDataSource defines the data source implementation.
type IncidentEpisodeInternalNoteDataSource struct {
    client *Client
}
...
func (d *IncidentEpisodeInternalNoteDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *IncidentEpisodeInternalNoteDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *IncidentEpisodeInternalNoteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data IncidentEpisodeInternalNoteDataSourceModel
    ...
    if hasId {
        readPath := "/incident-episode-internal-note/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read incident_episode_internal_note: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/incident-episode-internal-note/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list incident_episode_internal_note, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list incident_episode_internal_note: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for incident_episode_internal_note.")
            return
        }
        item = first
    }
