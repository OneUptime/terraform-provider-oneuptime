...
var _ datasource.DataSource = &PodmanHostFeedDataSource{}

func NewPodmanHostFeedDataSource() datasource.DataSource {
    return &PodmanHostFeedDataSource{}
}

// PodmanHostFeedDataSource defines the data source implementation.
type PodmanHostFeedDataSource struct {
    client *Client
}

// PodmanHostFeedDataSourceModel describes the data source data model.
type PodmanHostFeedDataSourceModel struct {
...
func (d *PodmanHostFeedDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_podman_host_feed"
}

func (d *PodmanHostFeedDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Log of everything that happened to this Podman host - creation, updates, owner changes and the rules that made them. Look up an existing podman_host_feed by `id` or by `name`.",
...
func (d *PodmanHostFeedDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *PodmanHostFeedDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data PodmanHostFeedDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a podman_host_feed.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/podman-host-feed/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No podman_host_feed found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read podman_host_feed: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/podman-host-feed/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list podman_host_feed, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list podman_host_feed: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No podman_host_feed found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one podman_host_feed matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for podman_host_feed.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["podmanHostFeedEventType"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["podmanHostFeedEventType"].(string); ok {
        data.PodmanHostFeedEventType = types.StringValue(val)
    } else {
        data.PodmanHostFeedEventType = types.StringNull()
    }
