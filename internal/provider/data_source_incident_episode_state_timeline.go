...
var _ datasource.DataSource = &IncidentEpisodeStateTimelineDataSource{}

func NewIncidentEpisodeStateTimelineDataSource() datasource.DataSource {
    return &IncidentEpisodeStateTimelineDataSource{}
}

// IncidentEpisodeStateTimelineDataSource defines the data source implementation.
type IncidentEpisodeStateTimelineDataSource struct {
    client *Client
}

// IncidentEpisodeStateTimelineDataSourceModel describes the data source data model.
type IncidentEpisodeStateTimelineDataSourceModel struct {
...
func (d *IncidentEpisodeStateTimelineDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_incident_episode_state_timeline"
}

func (d *IncidentEpisodeStateTimelineDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Change state of the incident episodes (Created to Acknowledged for example) Look up an existing incident_episode_state_timeline by `id` or by `name`.",
...
func (d *IncidentEpisodeStateTimelineDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *IncidentEpisodeStateTimelineDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data IncidentEpisodeStateTimelineDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a incident_episode_state_timeline.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/incident-episode-state-timeline/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No incident_episode_state_timeline found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read incident_episode_state_timeline: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list incident_episode_state_timeline: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No incident_episode_state_timeline found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one incident_episode_state_timeline matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for incident_episode_state_timeline.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["incidentStateId"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["incidentStateId"].(string); ok {
        data.IncidentStateId = types.StringValue(val)
    } else {
        data.IncidentStateId = types.StringNull()
    }
