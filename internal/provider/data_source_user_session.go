...
func (d *UserSessionDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Active user sessions with refresh tokens and device metadata for enhanced authentication security. Look up an existing user_session by `id` or by `name`.",
...
func (d *UserSessionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a user_session.",
        )
        return
    }
    ...
    if hasId {
        resp.Diagnostics.AddError("Lookup Not Supported", "user_session cannot be looked up by id: the API exposes no get endpoint. Use the name filter instead.")
        return
    } else {
        ...
        httpResp, err := d.client.Post(ctx, "/user-session/get-list", listBody)
        if err != nil {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No user_session found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one user_session matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
