...
var _ datasource.DataSource = &CallLogDataSource{}

func NewCallLogDataSource() datasource.DataSource {
    return &CallLogDataSource{}
}

// CallLogDataSource defines the data source implementation.
type CallLogDataSource struct {
    client *Client
}

// CallLogDataSourceModel describes the data source data model.
type CallLogDataSourceModel struct {
...
func (d *CallLogDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_call_log"
}

func (d *CallLogDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Logs of all the Call sent out to all users and subscribers for this project. Look up an existing call_log by `id` or by `name`.",
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "call_data": schema.StringAttribute{
                MarkdownDescription: "Content of the data that was sent in the call.",
                Computed: true,
            },
            ...
            "status": schema.StringAttribute{
                MarkdownDescription: "Status of the Call sent.",
                Computed: true,
            },
            "call_cost_in_usd_cents": schema.NumberAttribute{
                MarkdownDescription: "Call Cost in USD Cents.",
                Computed: true,
            },
...
func (d *CallLogDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *CallLogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data CallLogDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a call_log.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/call-log/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No call_log found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read call_log: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/call-log/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list call_log, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list call_log: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No call_log found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one call_log matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for call_log.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["callData"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
            data.CallData = types.StringValue(val)
        } else if val, ok := obj["value"].(string); ok {
            data.CallData = types.StringValue(val)
        } else if val, ok := obj["value"].(float64); ok {
            data.CallData = types.StringValue(fmt.Sprintf("%v", val))
        } else if jsonBytes, err := json.Marshal(obj); err == nil {
            data.CallData = types.StringValue(string(jsonBytes))
        } else {
            data.CallData = types.StringNull()
        }
    } else if val, ok := item["callData"].(string); ok {
        data.CallData = types.StringValue(val)
    } else {
        data.CallData = types.StringNull()
    }
