...
func (d *UserOnCallLogTimelineDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Timeline events for user on-call log. Look up an existing user_on_call_log_timeline by `id` or by `name`.",
...
func (d *UserOnCallLogTimelineDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a user_on_call_log_timeline.",
        )
        return
    }
    ...
    if hasId {
        resp.Diagnostics.AddError("Lookup Not Supported", "user_on_call_log_timeline cannot be looked up by id: the API exposes no get endpoint. Use the name filter instead.")
        return
    } else {
        ...
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No user_on_call_log_timeline found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one user_on_call_log_timeline matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
