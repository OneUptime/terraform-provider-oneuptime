...
var _ datasource.DataSource = &NetworkSiteAssignmentRuleDataSource{}

func NewNetworkSiteAssignmentRuleDataSource() datasource.DataSource {
    return &NetworkSiteAssignmentRuleDataSource{}
}

// NetworkSiteAssignmentRuleDataSource defines the data source implementation.
type NetworkSiteAssignmentRuleDataSource struct {
    client *Client
}
    ...
    resp.Schema = schema.Schema{
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "hostname_pattern": schema.StringAttribute{
                MarkdownDescription: "Devices whose hostname, SNMP system name or display name matches this wildcard pattern are assigned to the site.",
                Computed: true,
            },
...
func (d *NetworkSiteAssignmentRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NetworkSiteAssignmentRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data NetworkSiteAssignmentRuleDataSourceModel
    ...
    if hasId {
        readPath := "/network-site-assignment-rule/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read network_site_assignment_rule: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/network-site-assignment-rule/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list network_site_assignment_rule, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list network_site_assignment_rule: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for network_site_assignment_rule.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["subnetCidr"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["subnetCidr"].(string); ok {
        data.SubnetCidr = types.StringValue(val)
    } else {
        data.SubnetCidr = types.StringNull()
    }
