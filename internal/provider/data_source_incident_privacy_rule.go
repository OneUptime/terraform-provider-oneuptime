...
var _ datasource.DataSource = &IncidentPrivacyRuleDataSource{}

func NewIncidentPrivacyRuleDataSource() datasource.DataSource {
    return &IncidentPrivacyRuleDataSource{}
}

// IncidentPrivacyRuleDataSource defines the data source implementation.
type IncidentPrivacyRuleDataSource struct {
    client *Client
}
...
func (d *IncidentPrivacyRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *IncidentPrivacyRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *IncidentPrivacyRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data IncidentPrivacyRuleDataSourceModel
    ...
    if hasId {
        readPath := "/incident-privacy-rule/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read incident_privacy_rule: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/incident-privacy-rule/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list incident_privacy_rule, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list incident_privacy_rule: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for incident_privacy_rule.")
            return
        }
        item = first
    }
