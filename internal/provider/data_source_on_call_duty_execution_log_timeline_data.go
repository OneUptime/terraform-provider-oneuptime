    ...
    resp.Schema = schema.Schema{
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "user_notification_event_type": schema.StringAttribute{
                MarkdownDescription: "Type of event that triggered this on-call duty policy.. Permissions - Create: [No access - you don't have permission for this operation], Read: [Project Owner, Project Admin, Project Member, Viewer, On-Call Manager, Read On-Call Duty Policy Execution Log Timeline, Read All Project Resources], Update: [No access - you don't have permission for this operation]",
                Computed: true,
            },
...
func (d *OnCallDutyExecutionLogTimelineDataDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    ...
    if val, ok := onCallDutyExecutionLogTimelineDataResponse["triggered_by_alert_episode_id"].(string); ok {
        data.TriggeredByAlertEpisodeId = types.StringValue(val)
    }
    if val, ok := onCallDutyExecutionLogTimelineDataResponse["triggered_by_incident_episode_id"].(string); ok {
