...
var _ datasource.DataSource = &AlertInternalNoteDataSource{}

func NewAlertInternalNoteDataSource() datasource.DataSource {
    return &AlertInternalNoteDataSource{}
}

// AlertInternalNoteDataSource defines the data source implementation.
type AlertInternalNoteDataSource struct {
    client *Client
}
...
func (d *AlertInternalNoteDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *AlertInternalNoteDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AlertInternalNoteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data AlertInternalNoteDataSourceModel
    ...
    if hasId {
        readPath := "/alert-internal-note/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read alert_internal_note: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/alert-internal-note/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list alert_internal_note, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list alert_internal_note: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for alert_internal_note.")
            return
        }
        item = first
    }
