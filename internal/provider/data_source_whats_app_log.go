...
var _ datasource.DataSource = &WhatsAppLogDataSource{}

func NewWhatsAppLogDataSource() datasource.DataSource {
    return &WhatsAppLogDataSource{}
}

// WhatsAppLogDataSource defines the data source implementation.
type WhatsAppLogDataSource struct {
    client *Client
}

// WhatsAppLogDataSourceModel describes the data source data model.
type WhatsAppLogDataSourceModel struct {
...
func (d *WhatsAppLogDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
    resp.TypeName = req.ProviderTypeName + "_whats_app_log"
}

func (d *WhatsAppLogDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Logs of all the WhatsApp messages sent out to all users and subscribers for this project. Look up an existing whats_app_log by `id` or by `name`.",
        ...
        Attributes: map[string]schema.Attribute{
            ...
            "message_text": schema.StringAttribute{
                MarkdownDescription: "Text content of the WhatsApp message.",
                Computed: true,
            },
            ...
            "whats_app_message_id": schema.StringAttribute{
                MarkdownDescription: "Message ID returned by Meta's API.",
                Computed: true,
            },
            "status": schema.StringAttribute{
                MarkdownDescription: "Status of the WhatsApp message sent.",
                Computed: true,
            },
            "whats_app_cost_in_usd_cents": schema.NumberAttribute{
                MarkdownDescription: "WhatsApp Message Cost in USD Cents.",
                Computed: true,
            },
...
func (d *WhatsAppLogDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *WhatsAppLogDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data WhatsAppLogDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a whats_app_log.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/whatsapp-log/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No whats_app_log found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read whats_app_log: %s", err))
            return
        }
        ...
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list whats_app_log: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No whats_app_log found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one whats_app_log matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for whats_app_log.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["whatsAppMessageId"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["whatsAppMessageId"].(string); ok {
        data.WhatsAppMessageId = types.StringValue(val)
    } else {
        data.WhatsAppMessageId = types.StringNull()
    }
    ...
    if val, ok := item["whatsAppCostInUSDCents"].(float64); ok {
        data.WhatsAppCostInUsdCents = types.NumberValue(big.NewFloat(val))
    } else if obj, ok := item["whatsAppCostInUSDCents"].(map[string]interface{}); ok {
        if val, ok := obj["value"].(float64); ok {
            data.WhatsAppCostInUsdCents = types.NumberValue(big.NewFloat(val))
        } else {
