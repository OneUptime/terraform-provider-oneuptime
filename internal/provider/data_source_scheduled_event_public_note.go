...
var _ datasource.DataSource = &ScheduledEventPublicNoteDataSource{}

func NewScheduledEventPublicNoteDataSource() datasource.DataSource {
    return &ScheduledEventPublicNoteDataSource{}
}

// ScheduledEventPublicNoteDataSource defines the data source implementation.
type ScheduledEventPublicNoteDataSource struct {
    client *Client
}

// ScheduledEventPublicNoteDataSourceModel describes the data source data model.
type ScheduledEventPublicNoteDataSourceModel struct {
...
func (d *ScheduledEventPublicNoteDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
    resp.Schema = schema.Schema{
        MarkdownDescription: "Manage public notes for your scheduled event Look up an existing scheduled_event_public_note by `id` or by `name`.",
...
func (d *ScheduledEventPublicNoteDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ScheduledEventPublicNoteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data ScheduledEventPublicNoteDataSourceModel
    ...
    if hasId == hasName {
        resp.Diagnostics.AddError(
            "Invalid Lookup",
            "Exactly one of `id` or `name` must be set to look up a scheduled_event_public_note.",
        )
        return
    }
    ...
    if hasId {
        readPath := "/scheduled-maintenance-public-note/" + data.Id.ValueString() + "/get-item"
        ...
        if httpResp.StatusCode == http.StatusNotFound {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_event_public_note found with id %q.", data.Id.ValueString()))
            return
        }
        var itemResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read scheduled_event_public_note: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/scheduled-maintenance-public-note/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list scheduled_event_public_note, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list scheduled_event_public_note: %s", err))
            return
        }
        items, _ := listResponse["data"].([]interface{})
        if len(items) == 0 {
            resp.Diagnostics.AddError("Not Found", fmt.Sprintf("No scheduled_event_public_note found with name %q.", data.Name.ValueString()))
            return
        }
        if len(items) > 1 {
            resp.Diagnostics.AddError("Ambiguous Match", fmt.Sprintf("More than one scheduled_event_public_note matches name %q. Use the id attribute to disambiguate.", data.Name.ValueString()))
            return
        }
        first, ok := items[0].(map[string]interface{})
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for scheduled_event_public_note.")
            return
        }
        item = first
    }
