...
var _ datasource.DataSource = &OnCallDutyPolicyFeedDataSource{}

func NewOnCallDutyPolicyFeedDataSource() datasource.DataSource {
    return &OnCallDutyPolicyFeedDataSource{}
}

// OnCallDutyPolicyFeedDataSource defines the data source implementation.
type OnCallDutyPolicyFeedDataSource struct {
    client *Client
}
...
func (d *OnCallDutyPolicyFeedDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *OnCallDutyPolicyFeedDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *OnCallDutyPolicyFeedDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
    var data OnCallDutyPolicyFeedDataSourceModel
    ...
    if hasId {
        readPath := "/on-call-duty-policy-feed/" + data.Id.ValueString() + "/get-item"
        ...
        if err := d.client.ParseResponse(httpResp, &itemResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to read on_call_duty_policy_feed: %s", err))
            return
        }
        ...
        httpResp, err := d.client.PostBodyWithSelect(ctx, "/on-call-duty-policy-feed/get-list", listBody)
        if err != nil {
            resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list on_call_duty_policy_feed, got error: %s", err))
            return
        }
        var listResponse map[string]interface{}
        if err := d.client.ParseResponse(httpResp, &listResponse); err != nil {
            resp.Diagnostics.AddError("OneUptime API Error", fmt.Sprintf("Unable to list on_call_duty_policy_feed: %s", err))
            return
        }
        ...
        if !ok {
            resp.Diagnostics.AddError("OneUptime API Error", "Unexpected list response shape for on_call_duty_policy_feed.")
            return
        }
        item = first
    }
    ...
    if obj, ok := item["onCallDutyPolicyFeedEventType"].(map[string]interface{}); ok {
        if val, ok := obj["_id"].(string); ok && val != "" {
    ...
    } else if val, ok := item["onCallDutyPolicyFeedEventType"].(string); ok {
        data.OnCallDutyPolicyFeedEventType = types.StringValue(val)
    } else {
        data.OnCallDutyPolicyFeedEventType = types.StringNull()
    }
